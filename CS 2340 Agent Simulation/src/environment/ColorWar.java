...
public class ColorWar implements IEnvironment, IViewable {
	...
	public void reset() {
		...
		aStats = new Astats[Lagents.length]; // create agent statistics at each
												// index for each agent
	...
	public void setAgentLocation(int a, int x, int y) {
		...
		if (e[x][y].agent == -1) {// && e[x][y].available) {
			...
			e[x][y].agentScore = aStats[a].score; // sets the agent to that
													// agent number on the block
		}
		// else{
		// lose(a);
		// }
...
//			thread.start();
	...
	public double[][][] observeStructure(int a) {
		double[][][] state = new double[rView][cView][3]; // [availability,
															// color, agentScore
															// or 0]
		int x = aStats[a].x;
		int y = aStats[a].y;
		...
		for (int i = 0; i < rView; i++) {
			for (int j = 0; j < cView; j++) {
				int xval = leftx + i;
				int yval = lefty + j;
				if (xval >= gameSize || yval >= gameSize || xval < 0
						|| yval < 0) {
	...
	// @Override
	public double[] observe(int a) { // take every array in observed structure
										// and concatinate it end to end
		int count = 0;
	...
	// @Override
	public int actionRange(int a) { // /a constant # , largest # of moves that
									// agent will have available
		return 4;
	}
	...
	public void render(BufferedImage target) {
		...
		for (int i = 0; i < e.length; ++i)
			for (int j = 0; j < e[0].length; ++j) {
				...
				int x = gridToPixel(i);
				int y = gridToPixel(j);
				context.fillRect(x - (Constants.CELL_DISTANCE / 2)
						+ TILE_BUFFER, y - (Constants.CELL_DISTANCE / 2)
						+ TILE_BUFFER, Constants.CELL_DISTANCE
						- (2 * TILE_BUFFER), Constants.CELL_DISTANCE
						- (2 * TILE_BUFFER));
			}
		...
		for (int i = 0; i < aStats.length; ++i) {
			...
			context.fillOval(x - (AGENT_RADIUS) + 2, y - (AGENT_RADIUS) + 2,
					AGENT_RADIUS * 2 - 2, AGENT_RADIUS * 2 - 2);

			Stroke backup = context.getStroke();
			context.setStroke(stroke);
			context.setColor(ringColor);
			context.drawOval(x - (AGENT_RADIUS) + 2, y - (AGENT_RADIUS) + 2,
					AGENT_RADIUS * 2 - 2, AGENT_RADIUS * 2 - 2);
